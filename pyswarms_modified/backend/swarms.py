...
class Swarm_BS(object):
    ...
    * :func:`pyswarms.backend.generators.generate_velocity`: for generating velocities randomly.
    ...
    velocity = attrib(type=np.ndarray, validator=instance_of(np.ndarray))
	# # BS input
    # x1 = attrib(type=np.ndarray, validator = instance_of(np.ndarray))
    # x2 = attrib(type=np.ndarray, validator = instance_of(np.ndarray))
    # varminmax = attrib(type=np.ndarray, validator = instance_of(np.ndarray))
...
### BS ###
    # @varminmax.default
    # def varminmax_default(self):
    #     print(self.varminmax)
    #     return self.varminmax
