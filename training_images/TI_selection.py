...
class TI_selection():
  ...
  def __init__(self,setup,dataset,TI_name= "TI_1",nx = 60,ny = 60,nz = 1):
    ...
    path = base_path / "../../Output/training_images/{}/".format(self.TI_name)
  ...
  def plot_tof_best_models(self):
    ...
    all_cells_median = []
    for i in range(self.df_tof.shape[1]-2):
  ...
  def plot_best_model(self,random_TI = True,TI_id = 0,property = "PORO"):
    ...
    print("Plotting Training Image {}".format(model_id))
    ...
    if property == "tof":
        ...
        geomodel_path = str(self.setup["base_path"] / "../../Output/training_images/{}/".format(self.TI_name) / self.dataset / "all_models/INCLUDE/GRID.GRDECL")
        property_path = str(self.setup["base_path"] / "../../Output/"/ self.dataset / "all_models/INCLUDE" / property / "TI{}.GRDECL".format(TI_id))
  ...
  def clustering_tof_or_TI_props(self,n_neighbors = 30,min_dist = 0,n_components = 30, min_cluster_size = 10,
      ...
      if cluster_parameter == "TI_props":
          ...
          df_best_for_clustering = self.df_TI_props.drop(["TI_no"])
          ...
          df_best_for_clustering = self.df_tof.drop(columns = ["TI_no"])
  ...
  def save_best_clustered_TIs(self):
      """ save the best,previously selected reservoir models that are ready for 
        ...
        # path = str(self.setup["base_path"] / "../../Output/")+ "/"+ self.data_to_process[i] + "/"
        path = str(self.base_path / "../../Output/training_images/{}/".format(self.TI_name) / self.dataset)
