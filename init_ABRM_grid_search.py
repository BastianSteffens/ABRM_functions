...
def init():
    ...
    social_component = [0.1, 0.5,1.0,1.5,2.05] #1.994 #2.05
    cognitive_component = [0.1,0.3,1.0,1.5,2.05] # 2.05 ; 1.494 with 0 should all converge to the global minima,however good that is. 
    inertia = 0.9
    damping_factor = 0.99
    n_neighbors  =  [1,5,15,20,30]
    ...
    ###### Set modelling parameters for PSO ######
    ...
    varminmax = np.array([[1,4],[1,200],[1,200],[1,100],[1,100],[1,7],[1,7],
                          ...
                          [1,50],[1,1000],[1,50]])
    # varminmax = np.array([[1,200],[1,2000]])
    ...
    # var types str = 0, numeric =1
    parameter_type = [1,1]

    # misfit values
    # create curve and save resultign desired LC
    Phi_points_target = np.linspace(0, 1, num=11, endpoint=True)
    F_points_target = np.array([0, 0.45, 0.65, 0.75, 0.85, 0.95, 0.98, 0.99, 0.999, 0.99999, 1])
    ...
    g = GridSearch(ps.single.LocalBestPSO,n_particles = n_particles, dimensions= dimensions, options= options, objective_func= ABRM_functions.swarm, iters = n_iters, bounds = bounds, velocity_clamp= velocity_clamp)
